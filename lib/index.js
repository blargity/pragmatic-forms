...
var notInProd = function notInProd(cb) {
...
var initialFieldState = function initialFieldState(value) {
	return {
		...
		isDirty: false,
		error: undefined,
		valueType: typeof value === 'undefined' ? 'undefined' : (0, _typeof3.default)(value)
	};
};
...
var forceValueType = function forceValueType(value, type) {
	switch (type) {
		case 'string':
			return value.toString();
		case 'boolean':
			return (/^true|false$/.test(value) ? eval(value) : Boolean(value)
			);
		case 'number':
			return Number(value);
...
function configureForm(_ref) {
	var initFields = _ref.initFields,
	    ...
	    _ref$onFirstInteracti = _ref.onFirstInteraction,
	    onFirstInteraction = _ref$onFirstInteracti === undefined ? function () {} : _ref$onFirstInteracti;
	...
	return function decorator(WrappedComponent) {
		...
		return _temp = _class = function (_Component) {
			(0, _inherits3.default)(PragmaticForm, _Component);

			function PragmaticForm(props, context) {
				(0, _classCallCheck3.default)(this, PragmaticForm);

				var _this = (0, _possibleConstructorReturn3.default)(this, (PragmaticForm.__proto__ || (0, _getPrototypeOf2.default)(PragmaticForm)).call(this, props, context));

				_initialiseProps.call(_this);

				var initialData = config.initFields(props);
				_this.state.formFields = initialiseFormFields(initialData);
				return _this;
			}

			(0, _createClass3.default)(PragmaticForm, [{
				key: 'setState',
				value: function setState(changes, callback) {
					var _this2 = this;

					(0, _get3.default)(PragmaticForm.prototype.__proto__ || (0, _getPrototypeOf2.default)(PragmaticForm.prototype), 'setState', this).call(this, changes, function () {
						if (config && typeof config.onChange === 'function') {
							var _formData = getFormFieldsValues(_this2.state.formFields);
							config.onChange(_formData, _this2.props, _this2.formProps());
						}
						if (callback) {
							callback();
						}
			...
			return PragmaticForm;
		}(_react.Component), _class.displayName = 'PragmaticForm(' + (WrappedComponent.displayName || WrappedComponent.name) + ')', _initialiseProps = function _initialiseProps() {
			var _this3 = this;
			...
			this.updateField = function (name, value) {
				...
				var _state = _this3.state,
				    formFields = _state.formFields,
				    isPristine = _state.isPristine;

				var fieldItem = formFields[name];
				var newValue = value;
				...
				if (options.forceType) {
					...
					notInProd(function () {
						if (fieldItem.valueType !== (typeof value === 'undefined' ? 'undefined' : (0, _typeof3.default)(value))) {
							console.warn( // eslint-disable-line no-console
							'Value type of "' + name + '" has changed from "' + fieldItem.valueType + '" to "' + (typeof value === 'undefined' ? 'undefined' : (0, _typeof3.default)(value)) + '"', 'Use `forceType` to coerce the value to a supported type.');
						}
					});
				}

				if (isPristine) {
					onFirstInteraction(getFormFieldsValues(formFields), _this3.props, _this3.formProps());
				}

				_this3.setState({
					formFields: (0, _extends4.default)({}, formFields, (0, _defineProperty3.default)({}, name, (0, _extends4.default)({}, fieldItem, {
						value: newValue,
						error: undefined,
						isDirty: true
					}))),
			...
			this._createInputOnChange = function (name, options) {
				return function (event) {
					if (event.currentTarget.files) {
						_this3.updateField(name, event.currentTarget.files[0], options);
					} else {
						_this3.updateField(name, event.currentTarget.value, options);
					}
				};
			};

			this._createCheckOnChange = function (name, options) {
				return function (event) {
					_this3.updateField(name, event.currentTarget.checked, options);
				};
			};
			...
			this.formInputProps = function (options) {
				var name = options.name,
				    ...
				    changeOpts = (0, _objectWithoutProperties3.default)(options, ['name', 'type', 'value', 'checked']);

				if (!_this3.fieldProps[name]) {
					_this3.fieldProps[name] = {
						name: name,
						type: type,
						onChange: type === 'checkbox' ? _this3._createCheckOnChange(name, changeOpts) : _this3._createInputOnChange(name, changeOpts)
					};
				}
				...
				switch (type) {
					...
					case 'file':
						return (0, _assign2.default)(_this3.fieldProps[name], {
							files: [_this3._getFieldValueWithDefault(name, value)],
							disabled: disabled
						});
					default:
						return (0, _assign2.default)(_this3.fieldProps[name], {
			...
			this.formComponentProps = function () {
				return {
			...
			this.formComponent = function (props) {
				return _react2.default.createElement('form', (0, _extends4.default)({}, props, {
					onSubmit: _this3.handleSubmit,
					onReset: _this3.handleReset
				}));
			};
						...
						while (1) {
							switch (_context.prev = _context.next) {
								case 0:
									...
									errors = config.validate(formData, _this3.props, _this3.formProps());
								...
								case 5:
									...
									_context.prev = 6;
									_context.next = 9;
									return config.submit(formData, _this3.props, _this3.formProps());
								...
								case 9:
									...
									_this3.setState({
										...
										config.onSuccess(_result, _this3.props, _this3.formProps());
									});
									_context.next = 16;
									break;
								...
								case 13:
									...
									_this3.setState({
										...
										config.onError(_context.t0, _this3.props, _this3.formProps());
									});
					...
					return _regenerator2.default.wrap(function _callee2$(_context2) {
						while (1) {
							switch (_context2.prev = _context2.next) {
								case 0:
									event && event.preventDefault();
									initialData = config.initFields(_this3.props);
			...
			this.formProps = function () {
				...
				return {
					...
					getFormProps: _this3.formComponentProps,

					Form: _this3.formComponent,
